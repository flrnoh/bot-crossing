...
server's**, because two things write it (you archiving a thread, and the housekeeping sweep)
and every open tab reads it: a page that loaded an hour ago carries a stale copy, and if it
were allowed to hand that back on its next layout save it would silently resurrect everything
archived since. So a PUT cannot touch `archived`/`archivedAt` at all, `/api/archive` is the
only way in, and it hands the whole list back for the page to adopt. Writes to the file are
serialised through one chain, so the two callers cannot interleave a read-modify-write.
