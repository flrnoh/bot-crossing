 ...
 *
 * Two callers race for this file — a page PUTting its layout, and `/api/archive` recording
 * what you archived — and the file is rewritten whole, so an interleaved read-modify-write
 * would drop whichever change lost. The chain is per-process, which is all that is needed:
 * one server owns one file, and it is the only thing that writes it.
 */
 ...
 *
 * The archive list is different: it is written from two places (you archiving a thread, and
 * the housekeeping sweep) and read by every open tab. If the page owned it too, a tab that
 * loaded an hour ago would carry a stale copy and hand it back on its next layout save,
 * silently resurrecting everything archived since. So the server owns it outright: a PUT
 * cannot touch `archived`/`archivedAt` at all, and `/api/archive` is the only way in.
 */
...
export async function apiMiddleware(req, res, next) {
  ...
  try {
    ...
    if (url.pathname === '/api/archive' && req.method === 'POST') {
      ...
      // One thread or a batch. The sweep sends a batch so a quiet morning's worth of
      ...
      // The colony's own list first: it is the one that decides what you see, and it is
      // the only record a thread with no harness session has.
